...
export function Hero({ onOpenMediaKit }: HeroProps) {
  const [imgSrc, setImgSrc] = useState("/images/kashan-hero.png");
    ...
    <section id="top" className="relative min-h-[92vh] sm:min-h-screen pt-36 pb-28 sm:py-44 flex items-center justify-center bg-surface-base border-b border-border-strong/40 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 w-full relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          {/* Left Column: Bold Plain Identity */}
          <motion.div
          ...
          >
            ...
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-surface-strong/80 border border-border-strong text-sm text-text-secondary font-mono shadow-sm">
            ...
            <motion.p
              variants={fadeUp}
              className="text-lg sm:text-xl text-text-secondary font-normal max-w-2xl leading-relaxed"
            >
            ...
            {/* Metrics Badges */}
            <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-8 text-sm sm:text-base text-text-secondary font-mono pt-2">
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-4 pt-4">
              <a
                href="#pricing"
                className="bg-text-tertiary hover:bg-[#ff9d5c] text-surface-base font-bold text-base px-9 py-4 rounded-full transition-all shadow-glow hover:shadow-1 flex items-center gap-2.5 active:scale-95 group"
              >
              ...
              <a
                href="#work"
                className="text-text-primary hover:text-text-tertiary border border-border-strong hover:border-text-tertiary/50 bg-surface-muted/80 text-base font-semibold px-8 py-4 rounded-full transition-all flex items-center gap-2.5 shadow-sm"
              >
              ...
              <button
                ...
                className="text-sm font-semibold text-text-secondary hover:text-text-primary underline underline-offset-4 decoration-border-strong hover:decoration-text-tertiary transition-colors ml-auto sm:ml-4 py-2"
              >
                Get Media Kit (PDF) →
              </button>
            </motion.div>
          ...
          {/* Right Column: Kashan High-Res Portrait Frame */}
          <motion.div
          ...
          >
            <div className="relative w-full max-w-md">
              ...
              <div className="relative rounded-sm bg-surface-strong border border-border-strong p-4 shadow-1 overflow-hidden group">
                ...
                {/* Kashan High-Resolution Portrait with Smoothing */}
                <div className="relative aspect-[4/5] rounded-xs overflow-hidden bg-surface-base border border-border-strong/60 shadow-inner">
                  <Image
                    src={imgSrc}
                    ...
                    priority
                    unoptimized
                    onError={() => setImgSrc("https://yt3.ggpht.com/ytc/AIdro_k...")}
                    className="object-cover object-top group-hover:scale-105 transition-transform duration-700 contrast-[1.05] brightness-[1.02]"
                  />

                  {/* Elegant Gradient Overlay */}
                  <div className="absolute inset-0 bg-gradient-to-t from-surface-base via-transparent to-black/10 pointer-events-none" />
